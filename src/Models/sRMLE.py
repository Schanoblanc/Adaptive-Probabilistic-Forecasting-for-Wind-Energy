...
class StabalisedRMLEModel(IDBdSeqPrbPredModel):
    """
    This is one implementation for the Recursive Maximum Likelihood Estimation Model
    Use Matrix inversion identity to improvement the stability.
    """
    ...
    def __init__(self, lags,mu,var_z,P:np.ndarray, nu, alpha,epsilon=0.005,resolution=1001):
        self.lags = lags
        self.mu = mu
        self.var_z = var_z
        self.P = P
    ...
    def Fit(self, X):
        ...
        while( self._train_countdown>0):
            ...
            ### Calculate and Validate the change
            h= self._h(X)[:,np.newaxis] #shape(D,1)
            R_crm = h @ h.T # incremental of R
            if (not self._is_initialised): 
                self.P = np.linalg.inv(self.P + R_crm)
                self._is_initialised = True
            if(self._train_countdown>0): continue ## the pre-train process has not yet finish

            scaler = self._alpha / (1-self._alpha) + h.T @ self.P @ h
            newP = (self.P - self.P @ h @ h.T @ self.P / scaler) / self._alpha 
            change =  newP @ h
            ...
            self._trace_trP.append(np.trace(newP))
